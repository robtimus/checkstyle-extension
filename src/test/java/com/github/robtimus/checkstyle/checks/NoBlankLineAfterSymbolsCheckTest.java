/*
 * NoBlankLineAfterSymbolsCheckTest.java
...
@Encoding("UTF-8")
class NoBlankLineAfterSymbolsCheckTest {
    ...
    void testProcess(@TestResource("BlankLines.java") @AsLines List<String> lines) {
        ...
        NoBlankLineAfterSymbolsCheck check = newCheck();

        Set<Violation> violations = assertDoesNotThrow(() -> check.process(file, fileText));
        assertThat(violations, contains(
                violation(15, 0, "whitespace.blankLineAfterSymbols", TestMessages.whitespace.blankLineAfterSymbols("({["))
        ));
    }

    private NoBlankLineAfterSymbolsCheck newCheck() {
        NoBlankLineAfterSymbolsCheck check = new NoBlankLineAfterSymbolsCheck();
